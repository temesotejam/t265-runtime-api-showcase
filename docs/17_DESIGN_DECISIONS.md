...
## serialとroleをprivateにする理由

Serial numberは複数台運用には重要ですが、公開資料に載せる必要はありません。

公開docsでは、serial-based selectionやrole-based selectionの考え方だけを説明します。実際のserial number、role mapping、device固有設定はprivate boundaryの外に置きます。
